...
namespace SoftwareApproach.TestingExtensions
{
    public static partial class ExtensionMethods
    {
        ...
        {
            if (Char.ToUpper(source, CultureInfo.CurrentCulture) != Char.ToUpper(comparer, CultureInfo.CurrentCulture))
                AssertHelper.HandleFail("ShouldEqualIgnoringCase", message.AppendMessage(Resources.ActualExpectedString, source, comparer), parameters);
        ...
        {
            if (Char.ToUpper(source, CultureInfo.CurrentCulture) == Char.ToUpper(comparer, CultureInfo.CurrentCulture))
                AssertHelper.HandleFail("ShouldNotEqualIgnoringCase", message.AppendMessage(Resources.ActualExpectedString, source, comparer), parameters);
        ...
        public static char ShouldBePrintable(this char source, string message, params object[] parameters)
        {
            if (source < 32 || source > 126)
                AssertHelper.HandleFail("ShouldBePrintable",
                    message.AppendMessage(Resources.ActualString, String.Format(CultureInfo.CurrentCulture, "0x{0:X2}({0})", (byte)source)), 
                    parameters);
        ...
        {
            if (source >= 32 && source <= 126)
                AssertHelper.HandleFail("ShouldNotBePrintable", message.AppendMessage(Resources.ActualString, source), parameters);
