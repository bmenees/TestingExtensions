...
namespace SoftwareApproach.TestingExtensions
{
    ...
    public static partial class Testing
    {
        ...
        public static TExceptionType ShouldThrowException<TExceptionType>(Action action, string message, params object[] parameters) where TExceptionType : Exception
        {
            ...
            {
                if (typeof(TExceptionType).Equals(ex.GetType()))
                    return ex;
                else
                    AssertHelper.HandleFail("ShouldThrowException",
                        message.AppendMessage(Resources.ExceptionActualExpectedString, ex.GetType().ToString(), (typeof(TExceptionType)).ToString()),
                        parameters);
            }
            catch (Exception ex)
            {
            ...
            AssertHelper.HandleFail("ShouldThrowException",
                    message.AppendMessage(Resources.ExceptionNoneThrownString, (typeof(TExceptionType)).ToString()), parameters);
            return null;
        } 
    }
}
